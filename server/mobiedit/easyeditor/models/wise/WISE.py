...
class WISE(torch.nn.Module):
    ...
    def edit(self, config, tokens, act_mask=None, deact_mask=None):
        ...
        if getattr(eval(f"self.model.{self.layer}"), "editing_total_cnt") % self.config.save_freq == 0:
            print(f"generate_activation_mask")
            self.get_adapter_layer().generate_activation_mask(self.config.mask_ratio)

        # before_quan_logits = self.model(**tokens).logits
        ...
        # # calibrate
        # quan_model.calibrate(tokens, insert_module_name=self.layer+".down_proj")

        # after_quan_logits = self.model(**tokens).logits
        ...
        # before_quan_logits = before_quan_logits.view(len(tokens["input_ids"]), -1)
        # after_quan_logits = after_quan_logits.view(len(tokens["input_ids"]), -1)
        # cos_sim = torch.nn.functional.cosine_similarity(before_quan_logits, after_quan_logits, dim=1)
        # print(f"cos_sim:{cos_sim}")

        # self.model = quan_model
        ...
        if zo:
            # Zero-Order
            from .wise_zo import WISEZeroOrderTrainer

            zo_trainer = WISEZeroOrderTrainer(
                config=config,
                model=self,
                zo_eps=1e-5,
                device=f"cuda:{self.config.device}"
            ...
            losses = zo_trainer.forward_grad_step(tokens, act_mask, deact_mask)
            print(f"Zero-order optimization done. Final Loss = {losses[-1]:.4f}")
            ...
            for i in range(config.n_iter):
                ...
                if i == 0:
                    ...
                    optimizer = torch.optim.SGD([self.get_adapter_layer().new_weight], config.edit_lr, weight_decay=1e-2)
                ...
                print(f"ft_loss:{ft_loss}, act_loss:{act_loss}, loss:{loss}")
        ...
        # self.layer = "model.model.layers[23].mlp.down_proj"
