...
def apply_AlphaEdit_to_model(
  ...
  #-> Tuple[AutoModelForCausalLM, Dict[str, Any]]:
    ...
    if not os.path.exists(hparams.P_loc):
        ...
        print(f"hparams.rewrite_module_tmp.format(hparams.layers[-1]): {hparams.rewrite_module_tmp.format(hparams.layers[-1])}")
        print(f"Weight shape: {W_out.shape}")
        if "qwen" in hparams.model_name.lower() or "llama" in hparams.model_name.lower() or "gpt-j-6b" in hparams.model_name.lower():
