...
def execute_memit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        print(f"Layer {layer} took {format_time(time.time() - ks_time)} seconds to compute k/v pairs.")
        print(f"Writing {layer_ks.size(1)} key/value pair(s) into layer {layer}")

        # Compute residual error
        get_module_input_output_at_words_time = time.time()
        ...
        print(f"Layer {layer} took {format_time(time.time() - get_module_input_output_at_words_time)} seconds to compute cur_zs.")
        ...
        print(f"Layer {layer} took {format_time(time.time() - get_cov_time)} seconds to load covariance matrix.")
        ...
        print(f"Layer {layer} took {format_time(insert_time)} seconds to insert.")
