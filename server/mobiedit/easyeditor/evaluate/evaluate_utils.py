...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False):
    ...
    # prompts = [prompt + ' Think step by step.' for prompt in prompts]

    prompt_target = [prompt + ' ' + target for prompt, target in zip(prompts,targets)]
    print(f"prompt_target: {prompt_target}")
    ...
    with torch.no_grad():
        ...
        # print(f"answers: {answers}")
        # print(f"labels: {labels}")
        if isinstance(answers[0], list):  # 如果是batch，每个都是list
            decoded_answers = [tok.decode(ans, skip_special_tokens=True) for ans in answers]
            decoded_labels = [tok.decode(lbl, skip_special_tokens=True) for lbl in labels]
        else:  # 单个样本
            decoded_answers = tok.decode(answers, skip_special_tokens=True)
            decoded_labels = tok.decode(labels, skip_special_tokens=True)

        print(f"Decoded output (answers): {decoded_answers}")
        print(f"Decoded ground truth (labels): {decoded_labels}")
        if locality:
            return answers if type(answers[0]) is list else [answers,]
