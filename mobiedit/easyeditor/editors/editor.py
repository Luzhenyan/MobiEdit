...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        def replace_target_layers_with_linear(model, target_keys, weights, prefix=""):
            """递归替换目标模块为nn.Linear，只恢复weight，不要bias"""
            # print(f"model: {model}")
            # print(f"target_keys: {target_keys}")
            for name, module in model.named_children():
                full_name = prefix + name  # 正确构造完整路径
                # print(f"full_name: {full_name}")

                # 检查是不是我们要替换的层
                for target in target_keys:
                    if target == f"{full_name}.weight":
                        print(f"target: {target}")
                        print(f"full_name: {full_name}")
                        print(f"module: {module}")
                        if '8bit' in str(type(module)).lower():
                            # 是bnb量化Linear，替换成nn.Linear
                            weight = weights[target].to(module.weight.device)

                            new_linear = nn.Linear(module.in_features, module.out_features, bias=False).to(module.weight.device)
                            new_linear.weight.data.copy_(weight)

                            # 用新的Linear替换掉旧模块
                            setattr(model, name, new_linear)
                            print(f"[INFO] Replaced {full_name} with nn.Linear (no bias)")

                        break
                else:
                    # 递归子模块
                    replace_target_layers_with_linear(module, target_keys, weights, prefix=full_name + ".")
        ...
        if type(self.model_name) is str:
            ...
            if 't5' in self.model_name.lower():
                ...
                quan = hparams.quantize
                print(f"quan: {quan}")
                if quan == True:
                    print("===========load in int8===========")
                    model = AutoModelForCausalLM.from_pretrained(self.model_name,trust_remote_code=True, torch_dtype=torch_dtype, device_map=hparams.device)
                    if 'device_map' in model_kwargs:
                        model_kwargs.pop('device_map')
                    if hparams.alg_name == 'WISE':
                        # Retrieve weights that user desires to change in WISE
                        # print([name for name, param in model.named_parameters()])
                        weights = {
                            f"{hparams.unquantized_params[0]}": nethook.get_parameter(
                                model, f"{hparams.unquantized_params[0]}"
                            )
                        }
                    if hparams.alg_name == 'ROME':
                        weights = {
                            f"{hparams.rewrite_module_tmp.format(hparams.layers[0])}.weight": nethook.get_parameter(
                    ...
                    # 用fp32权重替换指定层
                    replace_target_layers_with_linear(self.model, list(weights.keys()), weights)

                else:
                    print("===========load in fp32===========")
                    self.model = AutoModelForCausalLM.from_pretrained(self.model_name,trust_remote_code=True, torch_dtype=torch_dtype, device_map=hparams.device)
                ...
                model = AutoModelForCausalLM.from_pretrained(self.model_name, trust_remote_code=True, **model_kwargs)
        ...
        if not hparams.model_parallel and hasattr(hparams, 'device') and hparams.alg_name != 'QLoRA' and getattr(self.model, 'is_loaded_in_4bit', False) and getattr(self.model, 'is_loaded_in_8bit', False):
    ...
    def edit_requests(self,
        ...
        def edit_func(request):
            ...
            if self.alg_name == 'IKE':
                ...
                icl_examples = None
            # # save model
            # path = '/home/u2023040027/EasyEdit/examples/output/'
            # # if os.path.exists(path+'edited_model.pth'):
            # #     os.remove(path+'edited_model.pth')
            # print("---save model---")
            # edited_model.save_pretrained('path')
        ...
        def edit_evaluation(all_metrics, request, edited_model, idx, test_generation, icl_examples, **kwargs):
            ...
            if self.alg_name == 'IKE':
                ...
                print(f"edited_model: {edited_model}, self.model_name: {self.model_name}, self.hparams: {self.hparams}, self.tok: {self.tok}, request: {request}, self.hparams.device: {self.hparams.device}, eval_metric: {eval_metric}, test_generation: {test_generation}")
