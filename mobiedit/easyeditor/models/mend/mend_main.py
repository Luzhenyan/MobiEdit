...
class MendRewriteExecutor:
    ...
    def init_model(self, model, tok, params: MENDHyperParams):
        ...
        if "model" in d:
            ...
            # 如果模型以state_dict形式保存
            self.alg.load_state_dict(
                {k.replace("gtn.", "mend."): v for k, v in d["state_dict"].items()}
            )
        elif isinstance(d, dict) and all(isinstance(k, str) for k in d.keys()):
            # 如果直接保存了状态字典
            self.alg.load_state_dict(
                {k.replace("gtn.", "mend."): v for k, v in d.items()}
            )
        else:
            # 如果是其他形式，尝试直接加载或提示错误
            print(f"无法识别的模型格式。请检查模型文件结构: {type(d)}")
            if hasattr(d, "state_dict"):
                self.alg.load_state_dict(
                    {k.replace("gtn.", "mend."): v for k, v in d.state_dict().items()}
                )
            else:
                raise ValueError(f"无法加载模型，未找到预期的键结构。可用键: {list(d.keys())}")
